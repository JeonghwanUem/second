...
public class SESUI {
		...
		while(true) {
			...
			switch(no){
				...
				case 3: 
					...
					//정상적으로 삭제되었는지 삭제할 데이터가 없는지 결과를 출력한다.
					if(deleteFlag) {
						System.out.println("정상적으로 삭제되었습니다.");
					}else {
						System.out.println("삭제할 데이터가 업습니다.");
					}
					break;
				...
				// 프로그램을 종료한다
				case 9: 
					System.exit(0);//외워야 한다. 프로그램을 종료 시키는 문구 
					break;
	...
	public void insertHuman(){
		while(true){
			...
			switch (no) {
				case 1: //1,2,3 다 같은 일을 하겠다는 뜻
				case 2:
				case 3:
					//사용자로부터 입력받아 객체를 생성한다.
					Human h = makeHuman(no);//리턴값이 휴맨
					//생성된 객체를 매니저로 전달하여 저장한다.
					boolean flag = manager.insertHuman(h);
					//정상등록되었는지 동일한 주민번호가 존재하는지 출력한다.
					if(flag) {
						System.out.println("정상적으로 되었습니다");
					}else {
						System.out.println("동일한 주민번호가 있습니다");
					}
	...
	public Human makeHuman(int type){
		...
		switch(type){
			case 1: System.out.print("|전공| ");
					...
					h = new Professor(name, age, jumin, major);//상향 형 변환이 이루어져 있다
					break;								//출력시 하양형 변환을 해야한다
